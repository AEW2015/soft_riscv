...
// Default to single precision unless otherwise specified
#ifndef DP
#ifndef SP
#define DP
#endif
...
// supports SIZE compile flag to set array size at compile time
#ifndef SIZE
#define SIZE        50
#endif
...
#ifdef STATIC_SIZE
#define LINPACK_STATIC_ALLOCATION
#endif // STATIC_SIZE

#ifdef LINPACK_STATIC_ALLOCATION
#define ARSIZE_2D (STATIC_SIZE * STATIC_SIZE)
#define MEMREQ_BY_REAL (ARSIZE_2D + STATIC_SIZE + STATIC_SIZE/2)
double fixed_alloc[MEMREQ_BY_REAL] = {0};
#else // LINPACK_STATIC_ALLOCATION
#endif // LINPACK_STATIC_ALLOCATION

void main(void) {
    char    buf[80];
#ifdef LINPACK_STATIC_ALLOCATION
    int arsize = STATIC_SIZE;
    long nreps;
    char temp[32];
    mempool = &fixed_alloc;
    int32_t memBytes = sizeof(fixed_alloc) + 512L;
#else // not LINPACK_STATIC_ALLOCATION
    int     arsize;
    ...
    // ensure arsize is always even
    arsize/=2;
    arsize*=2;
    // check for minimum array size
    if (arsize<10) {
        return;
    }
...
#ifndef LINPACK_STATIC_ALLOCATION
    //free(mempool);
#endif // LINPACK_STATIC_ALLOCATION
}
...
static double linpack(long nreps,int arsize)
    ...
    for (i=0;i<nreps;i++)
        {
        //printf("%d,%d dgefa a time: %f.\n",i,nreps,second());
        //printf("%lld\n\r",clock());
        //printf("0: %d\n\r",*(unsigned *)(0x10010008));
        //printf("1: %d\n\r",*(unsigned *)(0x10010018));
...
static REAL second(void)

    {
        ...
        return ((REAL)((REAL)result/(REAL)tmp));
    }
